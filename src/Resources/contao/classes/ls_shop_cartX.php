...
//			print_r($methodInfo);
	...
	protected function getData($database) {
        ...
        if($conn->connect_error) {
            $this->Template->echo = 'Verbindung zur Datenbank nicht möglich';
        }     
        $sql = 'SELECT * FROM '.$database;
        $result = $conn->query($sql);
        if(isset($result) == false) {
            $this->Template->echo = 'Datanbank abfrage nicht erfolgreich';
        }
        $array = array();
    ...
    function getMinOrderValue($product) {
    ...
    function productsAreGiven($couponID) {
    ...
    function getProdutsOfCoupon($couponID) {
